...
export class TimetableVersionService {
  ...
  async publish(id: string, userId: string): Promise<TimetableVersionEntity> {
    ...
    const currentPublished = await this.versionRepo.findPublished(
      version.semesterId,
    );
    if (currentPublished) {
  ...
  async cloneVersion(sourceVersionId: string): Promise<TimetableVersionEntity> {
    ...
    if (
      source.status !== TimetableVersionStatus.PUBLISHED &&
      source.status !== TimetableVersionStatus.ARCHIVED
    ) {
      throw new BadRequestException(
  ...
  async overwriteSlots(
    ...
    if (version.status !== TimetableVersionStatus.DRAFT) {
      throw new BadRequestException(
        'Chỉ có thể chỉnh sửa phiên bản ở trạng thái nháp',
      );
    }
