...
import { ImportTimetableDto, TimetableImportResultDto } from '../dto/import-timetable.dto';
...
export class TimetableImportController {
  ...
  @ApiOperation({
    ...
    description: 'Upload file Excel theo template chuẩn 6 cột (Lớp, Thứ, Tiết, Môn, GV, Phòng) cùng semesterId để import TKB. File phải có định dạng .xlsx/.xls và ≤ 10MB.',
  })
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        ...
        semesterId: { type: 'string', format: 'uuid', description: 'ID học kỳ đích' },
      },
    },
  })
  @ApiResponse({ status: 200, description: 'Kết quả import TKB', type: TimetableImportResultDto })
  @ApiResponse({ status: 400, description: 'File không hợp lệ (sai định dạng, quá lớn, hoặc rỗng)' })
  ...
  @ApiOperation({
    ...
    description: 'Tải file Excel mẫu với 6 cột: Lớp, Thứ, Tiết, Môn, Giáo viên, Phòng kèm dữ liệu mẫu.',
  })
