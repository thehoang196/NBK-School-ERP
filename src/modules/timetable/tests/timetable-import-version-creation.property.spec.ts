...
describe('Feature: timetable-management-features, Property 3: Version creation if and only if valid rows exist', () => {
  ...
  const rowSetArb: fc.Arbitrary<RowSpec[]> = fc
    ...
    .filter(([validRows, invalidRows]) => validRows.length + invalidRows.length >= 1)
      ...
      transaction: jest.fn().mockImplementation(async (cb: (manager: any) => Promise<any>) => {
        const mockManager = {
          create: jest.fn().mockImplementation((EntityClass: any, data: any) => {
            ...
            return { id: `slot-${Math.random().toString(36).slice(2)}`, ...data };
          }),
          save: jest.fn().mockImplementation(async (EntityClass: any, entity: any) => {
            if (EntityClass === TimetableVersionEntity) {
              return entity;
            }
  ...
  it('version is created if and only if successCount >= 1 (biconditional)', async () => {
    await fc.assert(
      fc.asyncProperty(rowSetArb, uuidArb, uuidArb, async (rows, schoolId, semesterId) => {
        ...
        const knownEntities = {
          ...
          roomCodes: [...new Set(validRows.filter((r) => r.roomCode).map((r) => r.roomCode))],
        };

        const { service, getVersionCreated } = createServiceWithMocks(schoolId, knownEntities);
        ...
        const result = await service.importFromExcel({ file, schoolId, semesterId });
  ...
  it('when all rows are invalid, no version is created (versionId is null)', async () => {
    ...
    const allInvalidRowsArb = fc
      .array(invalidRowArb, { minLength: 1, maxLength: 10 });

    await fc.assert(
      fc.asyncProperty(allInvalidRowsArb, uuidArb, uuidArb, async (rows, schoolId, semesterId) => {
  ...
  it('when at least one valid row exists, version is always created', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(atLeastOneValidArb, uuidArb, uuidArb, async (rows, schoolId, semesterId) => {
        const validRows = rows.filter((r) => r.isValid);
